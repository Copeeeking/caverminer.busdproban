var CONTRACT_ADDRESS = "0x89e84c38D647e68022B21e1530Bc6721977fA999";
var tokenAddr = "0xe9e7cea3dedca5984780bafc599bd69add087d56"; 
var referrer = '0';
var upline = '0';


var POOL = "0xA09D00150a1c938f6a23cC37Eae021F20f499adD";

var ABI = [{"inputs":[{"internalType":"address","name":"_timer","type":"address"},{"internalType":"address","name":"_ceoAddress","type":"address"},{"internalType":"address","name":"_devAddress","type":"address"}],"stateMutability":"nonpayable","type":"constructor"},{"anonymous":false,"inputs":[{"indexed":true,"internalType":"address","name":"previousOwner","type":"address"},{"indexed":true,"internalType":"address","name":"newOwner","type":"address"}],"name":"OwnershipTransferred","type":"event"},{"inputs":[{"internalType":"address","name":"adr","type":"address"}],"name":"beanRewards","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"},{"inputs":[{"internalType":"address","name":"ref","type":"address"},{"internalType":"uint256","name":"amount","type":"uint256"}],"name":"buyEggs","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[{"internalType":"uint256","name":"eth","type":"uint256"},{"internalType":"uint256","name":"contractBalance","type":"uint256"}],"name":"calculateEggBuy","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"},{"inputs":[{"internalType":"uint256","name":"eth","type":"uint256"}],"name":"calculateEggBuySimple","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"},{"inputs":[{"internalType":"uint256","name":"eggs","type":"uint256"}],"name":"calculateEggSell","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"},{"inputs":[],"name":"ceoAddress","outputs":[{"internalType":"address","name":"","type":"address"}],"stateMutability":"view","type":"function"},{"inputs":[],"name":"devAddress","outputs":[{"internalType":"address","name":"","type":"address"}],"stateMutability":"view","type":"function"},{"inputs":[],"name":"getBalance","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"},{"inputs":[{"internalType":"address","name":"adr","type":"address"}],"name":"getEggsSinceLastHatch","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"},{"inputs":[{"internalType":"address","name":"adr","type":"address"}],"name":"getMyEggs","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"},{"inputs":[{"internalType":"address","name":"adr","type":"address"}],"name":"getMyMiners","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"},{"inputs":[{"internalType":"address","name":"ref","type":"address"}],"name":"hatchEggs","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[],"name":"owner","outputs":[{"internalType":"address","name":"","type":"address"}],"stateMutability":"view","type":"function"},{"inputs":[],"name":"renounceOwnership","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[{"internalType":"uint256","name":"amount","type":"uint256"}],"name":"seedMarket","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[],"name":"sellEggs","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[{"internalType":"uint256","name":"beans","type":"uint256"}],"name":"setBeansInCan","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[],"name":"timer","outputs":[{"internalType":"address","name":"","type":"address"}],"stateMutability":"view","type":"function"},{"inputs":[{"internalType":"address","name":"newOwner","type":"address"}],"name":"transferOwnership","outputs":[],"stateMutability":"nonpayable","type":"function"}];
...
// moralis config
const serverUrl = "https://fx8yaa5r6vjr.usemoralis.com:2053/server";
const appId = "WHTk0ynJd1OC8C7A9I7rXdqIbHXLOsrGv9As68vI";
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            data['result'].forEach(element => {
                ...
                if ((method?.name == 'buyEggs' || method?.name == 'sellEggs') && (element.from == currentAddr?.toLowerCase() || element.to == currentAddr?.toLowerCase())) {
                    let txData = {
                        method: method.name,
                        amount: method.name == 'buyEggs' ? (method.params[1]?.value / 1e18).toFixed(2) : (method.params[0]?.value / 1e18).toFixed(2),
                        hash: element.hash,
    ...
    setInterval(async () => {
        ...
        if (contract) {
            ...
            if (currentAddr) {
                ...
                contract.methods.getMyEggs(currentAddr).call().then(res => {
                    $("#Tokens").text(res);
                    if (res > 0) {
                        console.log("Tokens: ", res);
                        contract.methods.calculateEggSell(res).call().then(res2 => {
                            $("#myrewards").html(` ${(res2 / 1e18).toFixed(2)} BUSD`);
                            console.log(res2);
                        })
                    }
                })
            ...
            tokenContract.methods.balanceOf(CONTRACT_ADDRESS).call().then(res => {
                $("#ba2").html(` ${(res / 1e18).toFixed(2)} BUSD`);
            })
...
const BuyMin = async () => {
    if (await Wallet() && contract) {
        ...
        if (res == "0x0000000000000000000000000000000000000000" && upline == '0') {
            contract.methods.buyEggs(res, web3.utils.toWei(trxspenddoc.value)).send({ from: currentAddr, gasPrice: gasPrice, })
        }
        else if (res == "0x0000000000000000000000000000000000000000" && upline != '0') {
            contract.methods.buyEggs(upline, web3.utils.toWei(trxspenddoc.value)).send({ from: currentAddr, gasPrice: gasPrice, })
        }
...
const reinvest = async () => {
    ...
    if (await Wallet() && contract) {
        ...
        if (res == "0x0000000000000000000000000000000000000000" && upline == '0') {
            contract.methods.hatchEggs(res).send({ from: currentAddr, gasPrice: gasPrice, })
        }
        else if (res == "0x0000000000000000000000000000000000000000" && upline != '0') {
            contract.methods.hatchEggs(upline).send({ from: currentAddr, gasPrice: gasPrice, })
        }
        else {
            document.querySelector('.w-form-ref-error').style.display = 'block'
...
function GetData(){
	return new Promise(function(resolve, reject) {
		...
		request.open('GET', 'https://theminer.io/js/out.json?', true);
